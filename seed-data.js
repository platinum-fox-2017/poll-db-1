...
function release_3_2() {
    db.each(`SELECT COUNT(politicianId) as totalVote, name FROM Politicians
...
function release_3_3() {
    ...
    FROM Votes LEFT JOIN Politicians ON Votes.politicianId = 
    Politicians.id WHERE Politicians.name LIKE 'Adam%' GROUP BY
    Politicians.name`,
        function (err, data) {
            console.log(data)
        })
...
function release_3_4() {
    db.each(`SELECT COUNT(*) as totalVote, name, party, location FROM Politicians
          LEFT JOIN Votes ON Politicians.id = Votes.politicianId
          GROUP BY name
...
function release_3_5() {
    db.each(`SELECT first_name, last_name, gender, age FROM Voters
