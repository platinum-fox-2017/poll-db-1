...
db.serialize(function(){
  ...
  //   FROM Politicians
  //   WHERE party = 'R' AND gradeCurrent
  //   BETWEEN 9 AND 11`,function(err, rows){
  //   if(err){
  //     console.log(err);
  //   } else {
  //     console.log(rows);
  ...
  // db.get(`SELECT COUNT(*) AS totalVote,
  // (SELECT name FROM Politicians
  //   WHERE id = politicianId) AS name
  // FROM Votes
  ...
  // 3. jumlah vote untuk politisi yang namanya ada kata 'Adam'
  // db.all(`SELECT name, COUNT(*) AS totalVote
  // FROM Votes
  // JOIN Politicians
  //   ON Votes.politicianId = Politicians.id
  // WHERE name LIKE '%Adam%'
  // GROUP BY name`, function(err,rows){
  //   if(err){
  //     console.log(err);
  //   } else {
  //     console.log(rows);
  ...
  // db.all(`SELECT count(*) AS totalVote, name, party, location, gradeCurrent
  // FROM Votes
  // JOIN Politicians
  //   ON Votes.politicianId = Politicians.id
  // GROUP BY name
  ...
  db.all(`SELECT firstName, lastName, gender, age
    FROM Voters
    JOIN Votes
